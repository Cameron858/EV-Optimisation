...
def test_that_higher_velocity_drains_battery_faster(v1, v2):
    ...
    def calc_time(v):
        ...
        return time_to_battery_drain(F_total, v, drivetrain_eff, battery_capacity_kWh)

    assert calc_time(v1) > calc_time(v2)


@pytest.mark.parametrize("b1, b2", [(1, 10), (10, 11), (10, 10.1)])
def test_that_larger_battery_capacity_increases_running_time(b1, b2):
    assert time_to_battery_drain(1000, 100, 1, b1) < time_to_battery_drain(
        1000, 100, 1, b2
    )


@pytest.mark.parametrize("eff1, eff2", [(1, 0.1), (1, 0.9), (1, 0.99), (0.5, 0.3)])
def test_that_lower_drivetrain_efficiencies_drains_battery_faster(eff1, eff2):
    assert time_to_battery_drain(1000, 100, eff1, 80) > time_to_battery_drain(
        1000, 100, eff2, 80
    )
...
def test_that_time_for_bettary_drain_raises_error_where_applicable(eta, raises):
    if raises:
        with pytest.raises(ValueError) as exc_info:
            time_to_battery_drain(1000, 100, eta, 80)
